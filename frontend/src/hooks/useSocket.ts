...
export const useSocket = (auctionId: string | undefined, enabled: boolean = false) => {
    ...
    useEffect(() => {
        const hasRequiredData = enabled && auctionId && isAuthenticated && accessToken;
        ...
        newSocket.on('connect', () => {
            console.log(`Successfully connected to auction socket: ${auctionId}`);
            setIsConnected(true);
            setSocket(newSocket);
        ...
        newSocket.on('connect_error', (err) => {
            console.error("Socket error detail:", err.message);
        ...
        return () => {
            if (newSocket) {
                console.log(`Cleanup: leaving auction room: ${auctionId}`);
    ...
    }, [auctionId, isAuthenticated, accessToken, enabled, refresh]);
