...
import type { FilterState } from '@/types/auction'
...
export const useAuctionStore = create<AuctionState>((set) => ({
    ...
    //         // Backend trả về list, mỗi auction đã có auction.item bên trong
    ...
    fetchAuctionById: async (auctionId: string) => {
        try {
            set({ loading: true });
            const data = await auctionService.fetchAuctionById(auctionId);
