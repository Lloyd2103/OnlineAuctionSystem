                ...
                <form id="item-form" onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        <div
                            className={`relative border-2 border-dashed rounded-xl transition-all cursor-pointer 
                                ${image.dragOver ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50 hover:bg-muted/30'}
                                ${image.preview ? 'h-48' : 'h-36'}`}
                            onDrop={image.handleDrop}
                            onDragOver={(e) => { e.preventDefault(); image.setDragOver(true); }}
                            onDragLeave={() => image.setDragOver(false)}
                            onClick={() => image.ref.current?.click()}
                        >
                            {image.preview ? (
                                <>
                                    <img src={image.preview} alt="Preview" className="w-full h-full object-cover rounded-xl" />
                                    ...
                                    <button
                                        type="button"
                                        onClick={(e) => { e.stopPropagation(); image.setPreview(''); image.setFile(''); }}
                        ...
                        <input ref={image.ref} type="file" accept="image/*" className="hidden"
                            onChange={(e) => e.target.files?.[0] && image.handleFile(e.target.files[0])} />
                    </div>
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {attributes.list.map((attr, index) => (
                                <div key={index} className="flex gap-2 items-center">
                                    <input
                                        type="text"
                                        value={attr.key}
                                        onChange={(e) => attributes.change(index, 'key', e.target.value)}
                                    ...
                                    <input
                                        type="text"
                                        value={attr.value}
                                        onChange={(e) => attributes.change(index, 'value', e.target.value)}
                                    ...
                                    <button
                                        ...
                                        disabled={attributes.list.length === 1}
